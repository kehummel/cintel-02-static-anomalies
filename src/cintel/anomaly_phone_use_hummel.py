...
DATA_FILE: Final[Path] = DATA_DIR / "phone_use_data_hummel.csv"
OUTPUT_FILE: Final[Path] = ARTIFACTS_DIR / "phone_anomalies_hummel.csv"
...
def main() -> None:
    ...
    LOG.info(f"Loaded {df.height} phone use data")
    ...
    LOG.info("Studying phone use in adults to find anomalies...")

    # x is age in years, so 21 is the lower limit for adults
    MIN_REASONABLE_X_VALUE: Final[float] = 21.0

    # y is phone usage before bed in minutes, so 120 minutes (2 hours) is the upper limit
    MAX_REASONABLE_Y_VALUE: Final[float] = 120.0

    # z is sleep duration in hours, so 10 is the upper limit
    MAX_REASONABLE_Z_VALUE: Final[float] = 10.0
    ...
    anomalies_df: pl.DataFrame = df.filter(
        (pl.col("age") < MIN_REASONABLE_X_VALUE)
        | (pl.col("phone_usage_before_sleep_minutes") >= MAX_REASONABLE_Y_VALUE)
        | (pl.col("sleep_duration_hours") >= MAX_REASONABLE_Z_VALUE)
    )
